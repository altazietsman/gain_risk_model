...
carbmaxs = function(b,c,LABA,vmax25,jmax25,r,dfe) {
  ...
  for (i in 1:enddf) {
    ...
    #create dataframe of cost model
    df = do.call(rbind, Map(data.frame, A=pl, B=l, C=wbrl))
    colnames(df)[1] = 'canopy_presure'
    colnames(df)[2] = 'PLC'
    colnames(df)[3] = 'K'
    #set PLC to run from zero and set K to match
    df$PLCnew = ((1-0)/(1-df$PLC[1]))*(df$PLC-1)+1
